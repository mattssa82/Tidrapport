...
// Filtrerar på år, projekt, fritext. Exporterar träffarna till CSV.
...
(function(){
  ...
  function loadDataToFlat(){
    let raw={};
    try{ raw=JSON.parse(localStorage.getItem(DATA_KEY)||"{}")||{}; }
    catch{ raw={}; }
    const flat=[];
    Object.keys(raw).forEach(mKey=>{ (raw[mKey]||[]).forEach(r=> flat.push(r)); });
    allRowsFlat = flat;
  }
  ...
  function runSearch(){
    ...
    lastResults = allRowsFlat.filter(r=>{
      ...
      const everything=[r.datum,r.projekt,r.kategori,r.tid,r.kortid,r.beskrivning].map(v=>String(v||"")).join(" ").toLowerCase();
  ...
  function exportResultsCSV(){
    ...
    lastResults.forEach(r=>{
      csv.push([r.datum||"",r.projekt||"",r.kategori||"",r.tid==null?"":r.tid,r.kortid==null?"":r.kortid,(r.beskrivning||"").replace(/\r?\n/g," ")].join(";"));
    });
