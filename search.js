...
// Filtrerar på år, projekt, fritext.
// Kan exportera träffarna till CSV.
...
(function(){
  ...
  document.addEventListener("DOMContentLoaded",()=>{
    ...
    if(ysel && ysel.value){
      runSearch(); // initial vy
    }

    const sb=get("searchBtn");
    if(sb) sb.addEventListener("click",runSearch);

    const eb=get("exportBtn");
    if(eb) eb.addEventListener("click",exportResultsCSV);
