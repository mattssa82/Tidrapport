/* app.js – Tidrapport (stabil build)
   Funktioner: Inmatning, tabell, larm, årsöversikt, export/delning (CSV/PDF/JSON),
   import (CSV/JSON), årsbyte, hoppa mellan månader, global sök (valfri), e-postutkast.
   Robust mot saknade DOM-element (körs utan fel även om en del UI saknas).
*/
(() => {
  // ===== Utils & konstanter =====
  const monthsSv = [
  ...
  const CSV_COLS = [
    "Datum","Ordinarie tid","Körtid","Projekt","Semester-tim","ATF-tim","Sjuk-tim",
    "Föräldraledig","VAB","Flextid","Övertid <2","Övertid 2>","Övertid-Helg","Traktamente","Beskrivning"
  ];
  ...
  const fmtBlankZero = n => {
    ...
    return (Math.round(v*100)/100)
      .toLocaleString("sv-SE", { minimumFractionDigits: 0, maximumFractionDigits: 2 });
  };
  ...
  let state = {
    ...
    data: load(STORAGE,{}),
    settings: load(SETTINGS,defSettings())
  };
  function defSettings(){
    return {
      company:"", name:"", emp:"", owner:"",
      year: new Date().getFullYear(),
      note:"", autoBackup:true, holidays:true
    };
  }
  function load(k, def){
  ...
  function saveData(){
    ...
    if(state.settings.autoBackup) localStorage.setItem("tidrapport_last_update", Date.now());
  }
  function saveCfg(){
  ...
  function initMenu(){
    ...
    // Inställningar (finns ev. inte i alla layouter)
    const map = {
      cfgCompany: "company", cfgName: "name", cfgEmp: "emp", cfgOwner: "owner",
      cfgYear: "year", cfgNote: "note"
    };
    Object.entries(map).forEach(([id,key])=>{
      const el = $(id);
      if(el) el.value = state.settings[key] ?? "";
    });
    const ab = $("cfgAutoBackup");
    if (ab) ab.checked = state.settings.autoBackup !== false;
  }
  window.changeMonth = () => {
    const sel = $("menuMonth"); if(!sel) return;
    state.month = Number(sel.value) || state.month;
    renderAll(); toggleMenu(false);
  };
  ...
  window.createYear = () => {
    const y = Number(prompt("Skapa nytt år:", (state.settings.year||new Date().getFullYear())+1));
    if(!y) return;
    state.settings.year = y; saveCfg(); renderAll();
  };
  window.deleteYear = () => {
    if(!confirm("År återställs till innevarande år (data ligger kvar).")) return;
    state.settings.year = new Date().getFullYear(); saveCfg(); renderAll();
  };
  ...
  function parseHourInput(v, allowEmpty=false){
    ...
    const n = toNumRaw(v); if(isNaN(n)) return NaN; return roundQuarter(n);
  }
  ...
  function clearInputs(){
    ["datum","tid","projekt","kortid","beskrivning"].forEach(id=>$(id)&&( $(id).value=""));
    const kat=$("kategori"); if(kat) kat.selectedIndex=0;
  }
  let editing=null;
  ...
  function yearFilter(r){
    return (new Date(r.datum)).getFullYear() === (Number(state.settings.year)||new Date().getFullYear());
  }
  function calcMonthTotals(m){
    ensureMonth(m);
    const res={_kortid:0,"Traktamente":0};
  ...
  function renderTable(){
    ...
    rows.forEach((r)=>{
      ...
      tr.innerHTML = `
        ...
        <td>${r.kategori}</td>
        <td>${fmtBlankZero(r.tid)}</td>
        <td>${fmtBlankZero(r.kortid)}</td>
        <td>${escapeHtml(r.beskrivning||"")}</td>
        <td class="actions">
  ...
  function renderYear(){
    const tb=$("yearBody"); if(!tb) return;
    tb.innerHTML="";
    for(let m=1;m<=12;m++){
      const t=calcMonthTotals(m);
      const tr=document.createElement("tr");
      tr.innerHTML=`<td>${cap(monthsSv[m-1])}</td>
        <td>${fmtBlankZero(t["Ordinarie tid"]||0)}</td>
        <td>${fmtBlankZero(t["Semester-tim"]||0)}</td>
        <td>${fmtBlankZero(t["ATF-tim"]||0)}</td>
        <td>${fmtBlankZero(t["Sjuk-tim"]||0)}</td>
        <td>${fmtBlankZero(t["Föräldraledig"]||0)}</td>
        <td>${fmtBlankZero(t["VAB"]||0)}</td>
        <td>${fmtBlankZero(t["Flextid"]||0)}</td>
        <td>${fmtBlankZero(t["Övertid <2"]||0)}</td>
        <td>${fmtBlankZero(t["Övertid 2>"]||0)}</td>
        <td>${fmtBlankZero(t["Övertid-Helg"]||0)}</td>
        <td>${fmtBlankZero(t["Traktamente"]||0)}</td>
        <td>${fmtBlankZero(t._kortid||0)}</td>`;
      tb.appendChild(tr);
    }
  }
  ...
  function renderAlarms(){
    ...
    const el=$("alarms");
    if(el) el.innerHTML = lines.length ? `<div class="pills">${lines.join("")}</div>` : "Inga varningar för denna månad.";
  }

  function renderAll(){
    renderTable();
    // Om global sökfält finns – uppdatera lista enligt aktuell data/år
  ...
  function sortedRows(months){
    ...
    months.forEach(m=>{
      ensureMonth(m);
      (state.data[m]||[]).filter(yearFilter).forEach(r=>out.push({...r,_m:m}));
    });
    out.sort((a,b)=>{
      const c=a.datum.localeCompare(b.datum); if(c) return c;
      const p=(a.projekt||"").localeCompare(b.projekt||""); if(p) return p;
  ...
  function headerInfo(scope){
    ...
    if(state.settings.note && state.settings.note.trim())
      parts.push(`Notering;${state.settings.note.replace(/\r?\n/g," ")}`);
    parts.push("");
    return parts.join("\r\n");
  }
  ...
  function makeBlobCSV(text){
    ...
    for(let i=0;i<s.length;i++){
      ...
      buf[bom.length+i*2]=code&0xFF; buf[bom.length+i*2+1]=(code>>8)&0xFF;
    }
  ...
  window.exportExcelStyleMonth = () => {
    // Samma innehåll, annan filbenämning – Excel läser BOM/UTF-16
    const text = buildCombinedCsv("month");
    const y=state.settings.year, m=String(state.month).padStart(2,"0");
    download(`tidrapport_${y}_${m}_excelstil.csv`, makeBlobCSV(text));
  };
  ...
  window.shareMonthReport = async () => {
    const text=buildCombinedCsv("month");
    const y=state.settings.year, m=String(state.month).padStart(2,"0");
    await tryShareOrDownload(`tidrapport_${y}_${m}_manadsrapport.csv`, makeBlobCSV(text), "Tidrapport – Månadsrapport");
  };
  window.shareYearReport = async () => {
    const text=buildCombinedCsv("year");
    const y=state.settings.year;
    await tryShareOrDownload(`tidrapport_${y}_arsrapport.csv`, makeBlobCSV(text), "Tidrapport – Årsrapport");
  };
  ...
  window.shareMonthPdfEmail = async () => {
    ...
    await tryShareOrDownload(`Tidrapport_${state.settings.year}_${String(state.month).padStart(2,"0")}.pdf`, blob, "Tidrapport – Månadsrapport (PDF)");
  };
  window.shareYearPdfEmail = async () => {
    const blob=makePdf("year");
    await tryShareOrDownload(`Tidrapport_${state.settings.year}.pdf`, blob, "Tidrapport – Årsrapport (PDF)");
  };

  // .eml utkast (enkel mailto) – dynamisk ämnesrad: "{Namn} Tidrapport {År} {Månad}"
  window.createEmlDraft = () => {
    const nm = (state.settings.name||"").trim();
    const subjName = nm ? nm+" " : "";
    const monthName = cap(monthsSv[state.month-1]||"");
    const subject = encodeURIComponent(`${subjName}Tidrapport ${state.settings.year} ${monthName}`);
  ...
  // ===== JSON Export/Import =====
  window.exportJSON = () => {
    const blob=new Blob([JSON.stringify({settings:state.settings,data:state.data},null,2)],{type:"application/json"});
    download(`tidrapport_backup_${state.settings.year}.json`, blob);
  };
  window.shareJSON = async () => {
    const blob=new Blob([JSON.stringify({settings:state.settings,data:state.data},null,2)],{type:"application/json"});
    await tryShareOrDownload(`tidrapport_backup_${state.settings.year}.json`, blob, "Tidrapport – Backup JSON");
  };
  ...
  window.APP = {
    editRow:(id,m)=>{
      ...
      set("datum",row.datum); set("tid",row.tid); set("projekt",row.projekt);
      set("kortid",row.kortid); set("beskrivning",row.beskrivning);
      const kat=$("kategori"); if(kat) kat.value=row.kategori;
      // Fokusera datumfältet om det finns – snabbare redigering
      $("datum")?.focus();
    },
    delRow:(id,m)=>{
  ...
  // ===== Global Sök (frivillig UI) =====
  // Stödjer sökning över alla månader i valt år på alla fält (datum, projekt, kategori, beskrivning m.m.)
  // Lägg till i HTML (valfritt):
  // <input id="searchQuery" placeholder="Sök i året (projekt, text, kategori, datum)"/>
  // <tbody id="searchBody"></tbody>
  function runSearch(){
    const qEl = $("searchQuery"), body = $("searchBody");
    if(!qEl || !body){ return; }
    ...
    months.forEach(m=>{
      ensureMonth(m);
      (state.data[m]||[]).filter(yearFilter).forEach(r=>rows.push({...r,_m:m}));
    });

    const results = q ? rows.filter(r=>{
      const hay = [
        r.datum || "",
        r.projekt || "",
        r.kategori || "",
    ...
    results.sort((a,b)=>a.datum.localeCompare(b.datum));
  ...
  // koppla events om sökfält finns
  $("searchQuery")?.addEventListener("input", runSearch);
