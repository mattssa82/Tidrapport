...
//  
// Innehåller:  
// - exportCSVImpl (månad, med projektsummering enligt 20251001)  
// - exportPDFImpl (månad till PDF A3 liggande)  
...
(function(){
  ...
  window.exportCSVImpl = function(rows, settings, year, month){
    try{
      ...
      // === Projektsummering enligt version 20251001 (FÅR EJ TAS BORT) ===
      csvLines.push("", "Projekt-summering:");
      csvLines.push("Projekt;Total tid (h)");

      const projectSums = {};
      useRows.forEach(r => {
        const p = (r.projekt || "").trim();
        if (!p) return;
        const h = parseFloat(r.tid) || 0;
        projectSums[p] = (projectSums[p] || 0) + h;
      });

      Object.keys(projectSums).sort().forEach(p => {
        csvLines.push(`${p};${projectSums[p].toFixed(2)}`);
      });
      // ================================================================
      ...
      alert("CSV-export klar (inkl. projektsummering).");
    }catch(err){
  ...
  window.exportYearImpl = function(rows, settings){
    try{
      const header=[
        ...
        "Semester","ATF","VAB","Sjuk","Föräldraledig","Trakt","Körtid"
      ];
