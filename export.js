...
//
// - exportCSVImpl (månad)
// - exportPDFImpl (månad till PDF, landscape A3)
// - exportYearImpl (årsöversikt CSV)
//
// Kolumnordning anpassad till:
/// Månad, Ordinarie, Körtid, Flex, ÖT<2, ÖT>2, ÖT-Helg, Semester, ATF, VAB, Sjuk, Föräld., Trakt

"use strict";

(function(){

  function classifyOvertimeBuckets(catLower){
    // return which overtime bucket
    if(catLower.includes("övertid") && catLower.includes("<2")) return "ot_lt2";
    if(catLower.includes("övertid") && (catLower.includes(">2") && !catLower.includes("helg"))) return "ot_gt2";
    if(catLower.includes("övertid") && catLower.includes("helg")) return "ot_helg";
    return null;
  }
  ...
  window.exportPDFImpl = function(rows, settings, year, month){
    try{
      ...
      if(!jsPDF){
        alert("jsPDF saknas – lägg till jsPDF + autoTable i sidan för PDF-export.");
        return;
      }
  ...
  // rows = ALLA rader (hela året)
  // Vi grupperar per månad och summerar.
  window.exportYearImpl = function(rows, settings){
    try{
      ...
      rows.forEach(r=>{
        ...
        const otType = classifyOvertimeBuckets(n);
        if(otType==="ot_lt2")  sumByMonth[m].ot_lt2+=h;
        if(otType==="ot_gt2")  sumByMonth[m].ot_gt2+=h;
        if(otType==="ot_helg") sumByMonth[m].ot_helg+=h;
        ...
        if(n.includes("föräld"))    sumByMonth[m].forald+=h;
